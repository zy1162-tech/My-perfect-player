...
(function(global) {
  ...
  // 轻量教练体系：只改变模拟节奏与攻防效率，不改写球员位置、评分或名单。
  var TEAM_SYSTEMS = {
  ...
  global.showLeagueIntel = function(done) {
    ...
    var modal = '<div class="team-picker-overlay" id="league-intel-modal"><div class="team-picker-modal" style="max-width:430px;">' +
      ...
      '<div style="padding:9px 12px 4px;font-size:11px;color:var(--text-dim);line-height:1.55;">交易完成后的球队快照：先看合同、轮换与离队风险，再决定下赛季打法。当前体系：<strong style="color:var(--orange);">' + system.icon + ' ' + system.name + '</strong></div>' +
  ...
  global.showTeamSystemChooser = function(done) {
