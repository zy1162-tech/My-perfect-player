...
assert.equal(context.NBA2K_DATA.GSW.find(player => player.name === 'Stephen Curry')?.ovr, 78, '2010-11 young-star calibration should lift Curry above the obsolete rookie rating');
assert.ok(context.NBA2K_DATA.GSW.find(player => player.name === 'Stephen Curry')?._youngStarOpeningFloor, 'Curry should be tagged as a calibrated young core');
